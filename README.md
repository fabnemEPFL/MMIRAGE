...
| Shard recovery | Kubernetes or Run:ai namespace, H100 access, `kubectl`, and a shared ReadWriteMany PVC. | Prepare workload and use `run_k8s.py status` after runs; native competitor manifests use `python experiments/shard_recovery/scripts/plan_native_competitor_recovery.py --framework all --condition all`. | [`experiments/shard_recovery/README.md`](experiments/shard_recovery/README.md) |
| Single-node H100 scaling | One node or pod with four visible H100 GPUs. | `bash experiments/single_node_h100_scaling/scripts/run_4gpu.sh --dry-run`; native competitor manifests use `python experiments/single_node_h100_scaling/scripts/plan_native_competitors.py --framework all --gpu-count all`. | [`experiments/single_node_h100_scaling/README.md`](experiments/single_node_h100_scaling/README.md) |
| AnonLib vs NeMo Curator comparison | One H100-80GB for the full shared-SGLang ChartQA run, plus isolated AnonLib, NeMo, and planned DataTrove environments. | Mock OpenAI-compatible VLM server and small ChartQA smoke workload documented in the runbook; DataTrove planned command uses `python experiments/nemo_curator_comparison/scripts/run_comparison.py --order datatrove --repetitions 1 --dry-run`. | [`experiments/nemo_curator_comparison/README.md`](experiments/nemo_curator_comparison/README.md) |
